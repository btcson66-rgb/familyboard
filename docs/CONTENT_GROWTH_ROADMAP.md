...
- After the first six 2026-08-22 batches, 21 indexable zh-TW content files exist. The verified local build contains 241 HTML routes and 223 indexable URLs; these figures are not a deployment claim.
...
5. **Second working-tool wave — local-ready:** appliance age calculator, maintenance cost tracker and recurring chore planner are independently written Taiwan Traditional Chinese editions with stricter input validation, explicit limitations and browser-tested outputs. Desktop/mobile interaction, accessibility and visual review passed on 2026-08-22.
6. **App workflow series — second batch local-ready:** product warranty tracking, household subscription management and document indexing are independently written zh-TW workflows tied to the current App. This batch also separates annualized subscription totals by currency, exposes review and reference details, keeps document notes out of handoff output, and shows readable asset/member names instead of internal IDs. Full preflight and desktop/mobile visual review passed on 2026-08-22.
7. **Further working-tool waves:** add tools in English/zh-TW pairs only after implementation and tests; use the third page slot for the supporting workflow when it adds distinct value.
8. **Evidence-led expansion:** after GSC begins returning queries, use real demand to reorder the backlog rather than publishing by calendar alone.
