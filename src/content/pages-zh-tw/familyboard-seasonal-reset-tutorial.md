...
locale: "zh-TW"
primaryIntent: "教台灣家庭用本機季節複查紀錄建立可交接的換季流程，不把它當成天氣或安全保證"
...
如果家庭在不同裝置或不同瀏覽器設定檔各有一份本機紀錄，先寫下這次使用的裝置代號、資料版本日期和來源代號，再決定要不要移轉。不要把兩份看起來相似的季節紀錄直接合併，也不要用重新整理來假設另一台裝置已更新。先在受控副本上確認新版本能讀取，再由家庭指定角色比較開放列、暫緩理由和下一個核點；若發現差異，保留兩份來源並記錄「待決定」，不要刪掉較舊版本來讓畫面看起來乾淨。這個步驟特別適合多人輪流照顧住家、長輩需要交接，或家中有一台共用平板的情況。
