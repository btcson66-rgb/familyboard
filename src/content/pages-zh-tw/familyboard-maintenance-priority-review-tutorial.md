...
faq:
  ...
  - question: "看到異常後應該先做什麼？"
    answer: "先依說明書或正式來源確認是否需要停用，再記錄家庭角色與下一步，不要自行拆修。"
---
...
複查前可以用五分鐘讓相關角色對齊：先讀最新觀察，再確認這次家庭要處理的是生活安排、來源詢問還是文件交接。請每個人只補充自己看見或從來源讀到的內容，將猜測改寫成「要向誰詢問的問題」。最後選一個負責角色和一個下次複查日期；若沒有正式回覆，狀態就保持等待，不要為了讓清單看起來完成而提前關閉。

## 第六點三：如何寫出可追溯的結果

結果欄應回答「哪個來源在什麼日期回覆了什麼」「家庭實際看到的變化是什麼」「還有哪個問題沒有答案」。例如可記錄「SERVICE-S1｜2026-08-26｜已回覆安排到訪｜等待到訪後觀察」，而不是「已修好」。若不同來源說法不一致，保留兩個來源代號與差異，交由家庭決定下一個詢問對象；FamilyBoard 不替你選出故障原因或安全結論。
