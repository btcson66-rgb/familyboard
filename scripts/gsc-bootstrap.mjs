...
if (!clientPath || !tokenPath) {
  throw new Error("Set GSC_OAUTH_CLIENT_PATH and GSC_OAUTH_TOKEN_PATH. Credentials are never stored in this repository.");
}
...
const listSites = async () => {
  ...
  if (!response.ok) throw new Error(`Search Console sites.list failed (${response.status}): ${json?.error?.message || json}`);
...
if (!entry && apply) {
  const { response, json } = await getJson(endpoint(`/${encodeURIComponent(siteUrl)}`), { method: "PUT" });
  console.log(`ADD_PROPERTY ${response.status} ${response.ok ? "PASS" : "FAIL"}`);
...
if (entry) {
  ...
  if (apply) {
    const sitemapEndpoint = endpoint(`/${encodeURIComponent(siteUrl)}/sitemaps/${encodeURIComponent(sitemapUrl)}`);
    const submit = await getJson(sitemapEndpoint, { method: "PUT" });
    console.log(`SUBMIT_SITEMAP ${submit.response.status} ${submit.response.ok ? "PASS" : "FAIL"}`);
    if (!submit.response.ok) console.error(submit.json?.error?.message || submit.json);
    const status = await getJson(sitemapEndpoint);
    console.log(`SITEMAP_READBACK ${status.response.status} ${status.response.ok ? "PASS" : "FAIL"}`);
  ...
  console.log("DRY_RUN Use --apply to attempt property addition after Google ownership is available.");
} else {
  console.log("BLOCKED Search Console ownership verification is required before sitemap submission.");
  process.exitCode = 2;
}
