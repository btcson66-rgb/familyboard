...
test("public SEO, keyboard and eight production tools work", async ({
  ...
  const sitemapManifest = JSON.parse(
    await readFile(
      new URL("../../src/generated/sitemap-pages.json", import.meta.url),
      "utf8",
    ),
  ) as Array<{ route: string; indexable: boolean; redirectTo?: string }>;
  ...
  ).toBe(indexableRoutes.has("/features/offline-household-organizer/"));
  for (const route of [
    ...
    expect(sitemap.includes(`<loc>https://familyboard.win${route}</loc>`)).toBe(
...
test("Traditional Chinese pages follow the indexability policy, stay localized and remain functional", async ({
  ...
  const indexabilityManifest = JSON.parse(
    await readFile(
      new URL("../../src/generated/sitemap-pages.json", import.meta.url),
      "utf8",
    ),
  ...
  const expectedPaths = sitemapManifest
    .filter((entry) => entry.indexable && !entry.redirectTo)
    .map((entry) => entry.route)
    .sort();

  expect(sitemapPaths).toEqual(expectedPaths);
  expect(sitemapPaths.length).toBeLessThanOrEqual(150);
  expect(sitemapPaths).toContain("/zh-tw/");
  expect(sitemapPaths).toContain("/zh-tw/tools/warranty-expiration-calculator/");
  expect(sitemapPaths).not.toContain("/app/");
  expect(sitemapPaths).not.toContain("/zh-tw/app/");
